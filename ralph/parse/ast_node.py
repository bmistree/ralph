...
class InitializerCallNode(_AstNode):
    def __init__(self,filename, variable_type_node,
                 initializer_identifier_node, line_number):
        super(InitializerCallNode,self).__init__(
            filename,ast_labels.INITIALIZER_CALL,line_number)

        self.variable_type_node = variable_type_node
        self.initializer_identifier_node = initializer_identifier_node

    def type_check_pass_one(self,struct_types_ctx):
        self.variable_type_node.type_check_pass_one(struct_types_ctx)
        self.initializer_identifier_node.type_check_pass_one(struct_types_ctx)

    def type_check_pass_two(self,type_check_ctx):
        self.variable_type_node.type_check_pass_two(type_check_ctx)
        self.initializer_identifier_node.type_check_pass_two(type_check_ctx)
...
class ServiceReferenceVariableTypeNode(VariableTypeNode):
    def __init__(self,filename,is_tvar,line_number):
        super(ServiceReferenceVariableTypeNode,self).__init__(
            filename,ast_labels.SERVICE_REFERENCE_TYPE,line_number)

        self.is_tvar = is_tvar
        self.type = ServiceReferenceType(self.is_tvar)
